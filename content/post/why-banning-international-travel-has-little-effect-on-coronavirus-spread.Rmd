...
I will get to international travel later, but for now this model will allow us to break down the Infected group to more accurately model spread dynamics. This model looks like
...
I'll gladly take suggestions for more accurate parameter values and I'm working on an interactive version of these plots. Nevertheless, putting this model in R and solving, we get:
...
Now I'm going to model international travel. Let's assume that there are two populations of roughly equal size. One has a head start on the other in terms of number of infected. Let's assume it has five times as many asymptomatic people as the US. So I'm going to assume the same parameters as before except the other population will have 100,000 infected instead of 20,000. Each population follows the same dynamics but I'll allow for travel between the two populations. Only the Susceptible, Asymptomatic, and Recovered travel. I'll set a parameter $\rho$, which is equal to the percent of people traveling each day. I will try out $\rho = 0.002$, which is roughly the percent of the US taking international travel daily. I will compare this to $\rho = 0$.

Note that the results of this model will depend on the fact that the disease has already spread somewhat to both populations. If one population doesn't have the disease at all, then of course completely stopping all travel to that country will stop it from getting the disease. This is the approach that [Micronesia has taken](https://slate.com/technology/2020/03/micronesia-drastic-coronavirus-travel-ban.html).
